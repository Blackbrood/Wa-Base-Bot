...
const {
	...
	proto,
	delay
} = require("baileys"); // Changed from @whiskeysockets/baileys to baileys
...
async function clientstart() {
	...
	// Prompt user to choose QR or Pairing Code
	const { say } = require('cfonts');
	const listcolor = ['red', 'blue', 'magenta'];
	const randomcolor = listcolor[Math.floor(Math.random() * listcolor.length)];
	
	const authMethod = await question(color(`<!> Choose authentication method:\n<1> QR Code\n<2> Pairing Code\nEnter 1 or 2: `, randomcolor));
	const usePairingCode = authMethod.trim() === '2';
	...
	// Send activation message to creator once connected
	bot.ev.on('connection.update', async (update) => {
		const { connection, lastDisconnect } = update;
		if (connection === 'open') {
			console.log(chalk.greenBright('Bot successfully connected!'));
			try {
				await bot.sendMessage('+2348078112891@s.whatsapp.net', {
					text: 'Dexter-md is now active and running successfully! 🚀'
				});
				console.log(chalk.greenBright('Activation message sent to +2348078112891'));
			} catch (err) {
				console.error(chalk.redBright('Failed to send activation message:'), err);
			}
		} else if (connection === 'close') {
			const reason = new Boom(lastDisconnect?.error)?.output.statusCode;
			if (reason === DisconnectReason.loggedOut) {
				console.log(chalk.redBright('Logged out, please re-authenticate.'));
			}
		}
	});
	...
	if (!bot.authState.creds.registered) {
		...
		say(`Create By King Caesar\nYOUTUBE: King Caesar\nTelegram: king_caesar\nInstagram: kingcaesar`, {
			font: 'console',
			align: 'center',
			gradient: [randomcolor, randomcolor]
		});
		...
		if (usePairingCode) {
			const phoneNumber = await question(color(`<!> TYPE YOUR WHATSAPP NUMBER STARTING WITH COUNTRY CODE (Don't start with 0)  ❌\n<✓> EXAMPLE: +2348078112891\n <+> Number: `, randomcolor));
			const code = await bot.requestPairingCode(phoneNumber.trim());
			console.log(color(`[ # ] Enter this pairing code into WhatsApp: ${code}`, randomcolor));
		} else {
			console.log(color(`[ # ] Scan the QR code displayed in the terminal with WhatsApp`, randomcolor));
		}
	}
